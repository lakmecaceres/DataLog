...
class DataLogGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        tab_widget.addTab(library_tab, "Library Info")
        tab_widget.addTab(indices_tab, "Indices & Metrics")
    ...
    def setup_basic_tab(self, tab):
        ...
        layout.addWidget(QLabel("Experiment Date:"), 0, 0)
        ...
        layout.addWidget(QLabel("Marmoset Name:"), 1, 0)
        layout.addWidget(self.marmoset_input, 1, 1)

        # Slab and tile numbers
        self.slab_input = QLineEdit()
        self.slab_input.setPlaceholderText("Enter numeric value")
        layout.addWidget(QLabel("Slab Number:"), 2, 0)
        layout.addWidget(self.slab_input, 2, 1)

        self.tile_input = QLineEdit()
        self.tile_input.setPlaceholderText("Enter numeric value")
        layout.addWidget(QLabel("Tile Number:"), 3, 0)
        layout.addWidget(self.tile_input, 3, 1)

        # Hemisphere selection
        self.hemisphere_input = QComboBox()
        self.hemisphere_input.addItems(["Left (LH)", "Right (RH)", "Both"])
        layout.addWidget(QLabel("Hemisphere:"), 4, 0)
        layout.addWidget(self.hemisphere_input, 4, 1)

        tab.setLayout(layout)

    def setup_sample_tab(self, tab):
        layout = QGridLayout()

        # Tile location multiselect
        self.tile_location_input = QComboBox()
        self.tile_location_input.addItems(["BS", "CX", "CB"])
        self.tile_location_input.setEditable(True)
        layout.addWidget(QLabel("Tile Location:"), 0, 0)
        layout.addWidget(self.tile_location_input, 0, 1)
        ...
        layout.addWidget(QLabel("Number of Reactions:"), 2, 0)
        layout.addWidget(self.rxn_number_input, 2, 1)

        # Sorter Initials
        self.sorter_initials_input = QLineEdit()
        self.sorter_initials_input.setPlaceholderText("Enter sorter's initials")
        layout.addWidget(QLabel("Sorter Initials:"), 3, 0)
        layout.addWidget(self.sorter_initials_input, 3, 1)
        ...
        layout.addWidget(self.project_name_input, 5, 2)
    ...
    def setup_library_tab(self, tab):
        ...
        layout.addWidget(QLabel("RNA Library Prep Date:"), 2, 0)
        ...
        layout.addWidget(QLabel("Percent cDNA > 400bp:"), 4, 0)
        layout.addWidget(self.percent_cdna_400bp_input, 4, 1)

        self.cdna_concentration_input = QLineEdit()
        self.cdna_concentration_input.setPlaceholderText("Comma-separated values (ng/µL)")
        layout.addWidget(QLabel("cDNA Concentration:"), 5, 0)
        layout.addWidget(self.cdna_concentration_input, 5, 1)
        ...
        layout.addWidget(QLabel("RNA Library Concentration:"), 6, 0)
        ...
        layout.addWidget(QLabel("ATAC Library Concentration:"), 7, 0)
        layout.addWidget(self.atac_lib_concentration_input, 7, 1)

        # Metrics
        self.expected_recovery_input = QLineEdit()
        layout.addWidget(QLabel("Expected Recovery:"), 8, 0)
        layout.addWidget(self.expected_recovery_input, 8, 1)

        self.nuclei_concentration_input = QLineEdit()
        layout.addWidget(QLabel("Concentration (nuclei/cells):"), 9, 0)
        layout.addWidget(self.nuclei_concentration_input, 9, 1)

        self.nuclei_volume_input = QLineEdit()
        layout.addWidget(QLabel("Volume (µL):"), 10, 0)
        layout.addWidget(self.nuclei_volume_input, 10, 1)
    ...
    def setup_indices_tab(self, tab):
        ...
        layout.addWidget(QLabel("RNA Indices:"), 0, 0)
        layout.addWidget(self.rna_indices_input, 0, 1)
        ...
        layout.addWidget(QLabel("RNA Library Sizes (bp):"), 2, 0)
        layout.addWidget(self.rna_sizes_input, 2, 1)
        ...
        layout.addWidget(QLabel("RNA Library Cycles:"), 4, 0)
        ...
        layout.addWidget(QLabel("ATAC Library Cycles:"), 5, 0)
        layout.addWidget(self.library_cycles_atac_input, 5, 1)
